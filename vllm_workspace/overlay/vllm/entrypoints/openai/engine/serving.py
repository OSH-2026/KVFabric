...
from vllm.tracing.utils import (
    contains_kvfabric_hint_headers,
    extract_kvfabric_hint_headers,
)
...
class OpenAIServing(BeamSearchOnlineMixin):
    ...
    async def _get_trace_headers(
        ...
        kvfabric_hint_headers: Mapping[str, str] = {}
        if os.environ.get("KVFABRIC_HINT_HEADER_TRACE", "1").strip().lower() not in {
            "0",
            "false",
            "no",
            "off",
        } and contains_kvfabric_hint_headers(headers):
            kvfabric_hint_headers = extract_kvfabric_hint_headers(headers)
        ...
        if kvfabric_hint_headers:
            return kvfabric_hint_headers
