...
class Scheduler(SchedulerInterface):
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                if tracker is not None and self.policy == SchedulingPolicy.FCFS:
                    pressure = self.kv_cache_manager.block_pool.cache_pressure_snapshot(
                        tracker.admission_head_window
                    )
                    should_defer = tracker.should_defer_request(
                        request_id=request_id,
                        prompt_tokens=request.num_tokens,
                        ...
                        already_deferred=request_id in kvfabric_deferred_requests,
                        deferrals_this_step=kvfabric_deferrals_this_step,
                        eviction_risk_ratio=float(
                            pressure["eviction_risk_ratio"]
                        ),
                    )
                    if should_defer:
                        ...
                        kvfabric_deferred_requests.add(request_id)
                        kvfabric_deferrals_this_step += 1
                        tracker.on_request_deferred(
                            request_id=request_id,
                            prompt_tokens=request.num_tokens,
                            hit_tokens=num_computed_tokens,
                            waiting_queue_size=(
                            ...
                            head_window_blocks=int(
                                pressure["head_window_blocks"]
                            ),
                            head_hashed_blocks=int(
                                pressure["head_hashed_blocks"]
                            ),
                        )
                        continue
                ...
                if tracker is not None:
                    tracker.on_request_scheduled(
                        ...
                        token_budget_before=token_budget_before_schedule,
    ...
    def _free_request(
        ...
        tracker = getattr(self.kv_cache_manager.block_pool, "kvfabric_lifecycle", None)
        if tracker is not None:
            tracker.on_request_finished(
                request_id=request_id,
                computed_tokens=int(getattr(request, "num_computed_tokens", 0) or 0),
                output_tokens=len(getattr(request, "_output_token_ids", []) or []),
