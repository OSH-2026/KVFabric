...
class SingleTypeKVCacheManager(ABC):
    ...
    def cache_blocks(
        ...
        if self.block_pool.kvfabric_lifecycle is not None:
            total_blocks = max(self.block_pool.num_gpu_blocks - 1, 0)
            pressure = self.block_pool.cache_pressure_snapshot(
                self.block_pool.kvfabric_lifecycle.admission_head_window
            )
            num_full_blocks = self.block_pool.kvfabric_lifecycle.limit_cache_blocks(
