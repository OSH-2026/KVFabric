...
- 模型预设：`Qwen/Qwen2.5-0.5B-Instruct` 和 `Qwen/Qwen3-8B`
...
本地 8GB 显存环境可先使用 `Qwen/Qwen2.5-0.5B-Instruct`，将 `max_model_len` 控制在 `1024` 到 `2048`，并通过增加共享前缀比例和请求到达模式放大策略差异。更高并发和更长上下文可在更大显存服务器上复跑关键实验。
