...
run_root="$VLLM_RUNTIME_DIR/remote_27b_policy/$(date +'%Y-%m-%d_%H%M%S')_qwen3_5_27b_policy_validation"
mkdir -p "$run_root"
echo "$run_root" > "$VLLM_RUNTIME_DIR/remote_27b_policy.latest"
...

{
  ...
  echo "kv_metrics=$KV_CACHE_METRICS sample=$KV_CACHE_METRICS_SAMPLE"
...
configs=(
  ...
  admission_control_probe.json
  template_family_revisit_cycles.json
  cache_pressure_hot_revisit.json
)

for cfg in "${configs[@]}"; do
  name="${cfg%.json}"
  echo "${name}_start=$(date -Is)" >> "$run_root/progress.log"
  bash experiments/prebenchmark_validation/scripts/run_kvfabric_ab_smoke.sh \
    qwen3_5_27b \
    "experiments/prebenchmark_validation/configs/$cfg" \
    > "$run_root/${name}.log" 2>&1
  grep 'KVFabric A/B output:' "$run_root/${name}.log" \
    | tail -1 > "$run_root/${name}.run_dir" || true
  echo "${name}_done=$(date -Is)" >> "$run_root/progress.log"
done
